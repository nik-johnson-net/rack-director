...
async fn create_role(
    ...
    state.os_store.get(req.os_id)?;

    let role = state.roles_store.create(
        &req.name,
...
async fn list_roles(
    ...
    let roles = state.roles_store.list_with_os()?;
    Ok(Json(roles))
}
...
async fn get_role(
    ...
    let role = state.roles_store.get_with_os(id)?;
    Ok(Json(role))
}
...
async fn update_role(
    ...
    if let Some(os_id) = req.os_id {
        state.os_store.get(os_id)?;
    }

    let role = state.roles_store.update(
        id,
        req.name.as_deref(),
        req.description.as_deref(),
        req.os_id,
...
async fn list_role_devices(
    ...
    let devices = state.roles_store.list_devices_with_role(role_id)?;
    Ok(Json(devices))
}
...
async fn assign_role(
    ...
    // Verify role exists
    state.roles_store.get(req.role_id)?;

    // Verify device exists
    state.director.get_device(&uuid).await?;

    state.roles_store.assign_to_device(&uuid, req.role_id)?;
...
async fn get_device_role(
    ...
    let role = state.roles_store.get_device_role(&uuid)?;
    Ok(Json(role))
}

...
fn convert_old_layout(value: &serde_json::Value) -> Result<serde_json::Value> {
    ...
    for (idx, part) in partitions.iter().enumerate() {
        ...
        device_map.entry(base_device).or_default().push(new_partition);
    }
...
mod tests {
    ...
    fn test_convert_old_layout_simple() {
        ...
        assert_eq!(disk.get("partition_table").unwrap().as_str().unwrap(), "gpt");
        ...
        assert_eq!(partitions[0].get("filesystem").unwrap().as_str().unwrap(), "vfat");
        assert_eq!(partitions[0].get("mount_point").unwrap().as_str().unwrap(), "/boot/efi");

        // Second partition
        assert_eq!(partitions[1].get("size").unwrap().as_str().unwrap(), "rest");
        assert_eq!(partitions[1].get("filesystem").unwrap().as_str().unwrap(), "ext4");
        assert_eq!(partitions[1].get("mount_point").unwrap().as_str().unwrap(), "/");
    }
    ...
    fn test_convert_old_layout_nvme() {
        ...
        let disks = result.get("disks").unwrap().as_array().unwrap();
        assert_eq!(disks.len(), 1);
        assert_eq!(disks[0].get("device").unwrap().as_str().unwrap(), "/dev/nvme0n1");
    }
}
